...
2. **화면 구현 완료** — 홈·단어장(가리기/스와이프 5단계/단어 셀 탭 TTS/예문 바텀시트)·복습·테스트·용돈 장부·발음 체크 장부(셀프 해소+재체크 자동 복귀)·설정. Expo Go 실기기 구동 확인됨.
3. 프로젝트는 **Expo SDK 54 고정** (스토어 Expo Go 호환 — 상세는 horang-worker-guide 스킬). 시스템 Python 3.9. anthropic SDK 0.116.0 --user 설치됨.
4. 문서 미결로 기본값 정한 것들 (사용자 조정 가능): 테스트 쓰기문제 비율 30%(lib/reviewQueries.ts WRITING_RATIO), 점수→용돈 매핑(lib/incomeQueries.ts DEFAULT_INCOME_RULES), 엿보기 1.4초(app/day PEEK_DURATION_MS), 사이렌 시점 10초(app/day SIREN_AT_REMAINING_MS).
5. **다음 후보**: 실기기 전 화면 QA → 사용자 피드백 반영 → 단어장 예문 컬럼 추가 여부 결정 → 배포 준비(EAS 빌드, 유료 Apple 계정 필요 시점에 제안).
