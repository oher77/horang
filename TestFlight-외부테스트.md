...
## 6. 테스터에게 보내는 글 — **두 덩어리로 나눈다**

> ⚠️ **질문을 What to Test에 넣지 말 것.** 테스터는 이 글을 앱을 **열기 전에** 읽는다.
> 거기에 "호랑이를 눌렀을 때 알아차렸나요?" 같은 질문을 적으면 답을 미리 알려주는 셈이고,
> 그러면 **발견 가능성을 물을 수 없게 된다** — 이 단계의 존재 이유가 사라진다.
> (2026-08-23 정정. 그 전 버전의 3번 질문이 정확히 이 실수였다.)
>
> 그래서 **안내는 앱을 열기 전에(①), 질문은 다 써본 뒤에(②)** 따로 보낸다.

### ① What to Test (App Store Connect에 붙여넣기)

TestFlight → Builds → 해당 빌드 선택 → **What to Test**

```
처음 쓰시는 그대로, 설명 없이 자유롭게 만져 봐 주세요.
정해진 순서 없이 마음 가는 대로 써 보시면 됩니다.

- 헤맨 지점, 기대와 다르게 동작한 지점을 편하게 알려주세요.
- 글자가 작거나 커서 불편하지 않았는지도 봐 주시면 좋습니다.

버그를 만나면 TestFlight 앱에서 스크린샷을 찍어 바로 피드백을 보낼 수 있습니다.
스크린샷 > 공유 버튼 > 하단의 '베타 피드백 공유'

이번 버전에서 고친 것
· 일부 기기 설정에서 화면 요소가 어긋나거나 동작이 멈추던 문제

문의: heruse@gmail.com
```

> **고친 내용을 "동작 줄이기"·"호랑이"라고 쓰지 않은 것도 의도적이다** — 그렇게 쓰면
> 그 기능을 찾아 나서게 된다. "일부 기기 설정"까지가 스포일러 없이 말할 수 있는 한계다.
>
> **기기 종류는 묻지 않는다.** TestFlight 앱으로 보낸 피드백에는 기기·OS·화면 크기·로케일이
> 자동으로 붙는다(App Store Connect의 피드백 화면에서 보인다). 아래 ②에서만 묻는다.

### ② 다 써본 뒤 따로 보낼 질문 (카톡·메일)

```
써 보셨다니 감사합니다. 몇 가지만 여쭐게요.

1. 홈 화면에서 가장 먼저 누른 곳은 어디였나요? 기대한 화면이 나왔나요?
2. "오늘 공부를 시작하는 입구"를 찾는 데 얼마나 걸렸나요?
3. 쓰다가 뜻밖에 재미있었던 순간이 있었나요? 무엇을 하다가였나요?
4. 단어장 → 테스트까지 해 보셨다면, 막히거나 헷갈린 지점이 있었나요?
5. 글자 크기는 읽기 편했나요?
6. 아이가 혼자 계속 쓸 것 같나요? 안 쓸 것 같다면 무엇 때문일까요?
7. 사용하신 기기를 알려주세요 (예: iPhone 13).
```

> **3번이 핵심이고, 호랑이를 이름으로 부르지 않는 게 요점이다.** 여기서 호랑이 이야기가
> 안 나오면 그 자체가 답이다 — 발견되지 않았다는 뜻이고, 홈 화면 진입점 설계를 다시 봐야 한다.
> 1~3번은 **딸로는 검증이 불가능한 항목**(제작 과정을 봐서 앱을 이미 안다)이라
> 외부 테스트를 하는 진짜 이유다.
>
> 7번을 여기서 묻는 이유: 카톡·메일에는 TestFlight 메타데이터가 붙지 않는다.

### 레이아웃이 이상하다는 제보를 받으면 — **접근성 설정부터 물을 것**

TestFlight 메타데이터에 기기·OS는 나오지만 **아래 두 설정은 안 나온다.** 물어봐야만 안다.

- 설정 → 디스플레이 및 밝기 → **텍스트 크기** (기본은 슬라이더 7칸 중 4번째 = 배율 1.0)
- 설정 → 손쉬운 사용 → 동작 → **동작 줄이기**

2026-08-22 실기기 QA에서 이 두 개 때문에 홈 글자 어긋남·호랑이 무반응·로켓 미표시가
한꺼번에 발생했고, 기기 차이로 오해해 원인 규명에 하루가 들었다(커밋 `9ddc856`에서 수정).
지금은 코드가 두 설정을 차단하지만, 비슷한 제보가 오면 **이것부터 확인하는 게 가장 빠르다.**
